...
async function carregarDados() {
    try {
        console.log("Buscando locais no servidor...");
        const resposta = await axios.get("http://localhost:3001/api/locais");

        console.log("Recebido da API:", resposta.data);

        // Substitui os locais fixos pelos locais da API
        locais.length = 0; // esvazia o array
        resposta.data.forEach(item => locais.push(item)); // adiciona os locais da API

        // Recria os marcadores no mapa
        criarMarcadores();

    } catch (erro) {
        console.error("Erro ao carregar locais da API:", erro);
    }
}
// Dados de exemplo
const locais = [
